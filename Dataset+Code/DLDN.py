...
for mm in range(1, 6):
    for cv in range(1, 4):
        ...
        k = 5
        max_batches = 2500      # how many batches if the early stopping condition not satisfied
        N_FEATURES = 200
        FEATURE_SHAPE = (200,)
        dataset_label = "RNA_PRO"
        data_batch_size = 32
        mask_batch_size = 32    # final batch_size is data_batch_size x mask_batch_size
        s = 150                 # size of optimal subset that we are looking for
        ...
        early_stopping_patience = 200  # how many patience batches (after phase 2 starts)
        ...
        for j in range(time):
            ...
            for i in range(k):
                ...
                # Create a dense operator net
                fs.create_dense_operator([60, 30, 20, 2], "softmax", metrics=[keras.metrics.CategoricalAccuracy()],
                                         error_func=K.categorical_crossentropy)
                # Ealy stopping activate after the phase2 of the training starts.
                fs.operator.set_early_stopping_params(phase_2_start, patience_batches=early_stopping_patience,
                                                      minimize=True)
                # Create a dense selector net
                ...
                predict = fs.operator.test_one(X_te, optimal_mask[None, :], y_te)
                label = y_te[:, 1]
                score = predict[:, 1]
        ...
        f_1 = open('res_cv.txt', 'a')
        f_1.write('res_data/data' + str(mm) + 'cv' + str(cv) + ':\n')
